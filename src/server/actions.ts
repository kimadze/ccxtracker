...
export async function createPortfolio(input: unknown): Promise<ActionResult> {
  ...
  try {
    await seedAssets();
    const p = await portfolioService(getDb(), user.id).create(input);
...
export async function deletePosition(
  ...
  const user = await requireUser();
  try {
    await portfolioService(getDb(), user.id).deletePosition(
      portfolioId,
      assetId,
      revision,
    );
...
export async function renamePortfolio(
  ...
  const user = await requireUser();
  try {
    await portfolioService(getDb(), user.id).rename(id, input);
    revalidatePath("/portfolios", "layout");
    return { ok: true };
  } catch (e) {
    return { ok: false, error: userError(e) };
...
export async function searchAssets(query: string) {
  ...
  try {
    if (
      ...
      return { ok: false as const, error: "შეიყვანეთ მინიმუმ 2 სიმბოლო." };
    if (!(await consumeRateLimit(`asset-search:${user.id}`, 10, 60000)))
      return {
        ok: false as const,
        error: "ძიების ლიმიტი მიღწეულია. სცადეთ ერთი წუთის შემდეგ.",
      };
