...
export function Attachments({
  ...
  return (
    <section className="panel space-y-5 p-6">
      ...
      {files.map((f) => (
        <div key={f.id} className="flex items-center justify-between gap-3">
          <a
            ...
            className="break-all text-xs text-brand"
          >
            {f.name}
          </a>
          <button
      ...
      {enabled && journalId && files.length < 5 ? (
        <form
          className="space-y-4"
          onSubmit={async (e) => {
            e.preventDefault();
        ...
        <p className="text-xs text-muted">
          {!enabled
            ? "დანართების საცავი ჯერ არ არის დაკავშირებული."
            : !journalId
              ? "დანართის დამატებამდე შეინახეთ ჟურნალი."
              : "დანართების მაქსიმალური რაოდენობა მიღწეულია."}
        </p>
      )}
      ...
      >
        <button
          ...
          onClick={async () => {
            setPending(true);
            try {
              const response = await fetch(`/api/attachments/${deleting}`, {
                method: "DELETE",
              });
              if (!response.ok) setError("წაშლა ვერ მოხერხდა.");
              else {
                setDeleting(null);
                router.refresh();
              }
