...
export function Overview({
  ...
  const allocationValue = allocationPositions.reduce(
  ...
  const slices = allocationPositions.map((position, index) => ({
    ...
    value: allocationValue
      ? (Number(position.value ?? 0) / allocationValue) * 100
      : 0,
  ...
  return (
    <div className="dashboard-space">
      ...
      <section className="dashboard-metrics">
        <DashboardMetric
          icon={<Wallet size={20} />}
          label="სრული პორტფელი"
          value={money(s.value)}
          change={`${money(s.totalPnl)}  ${percentage(s.value && s.totalPnl ? percent(s.totalPnl, s.value) : null, true)}`}
        />
        <DashboardMetric
          icon={<CalendarDays size={20} />}
          label="ლიკვიდობის რეზერვი"
          value={money(s.liquidity)}
          change={percentage(liquidityShare)}
        />
        <DashboardMetric
          icon={<Target size={20} />}
          label="რეალიზებული P/L"
          value={money(s.realizedPnl)}
          change="დახურული პოზიციები"
        />
        <DashboardMetric
          icon={<Coins size={20} />}
          label="აქტიური მონეტები"
          value={String(s.positions.length).padStart(2, "0")}
          change={`${positions.length} შეფასებული აქტივი`}
        />
      </section>
      ...
      <section className="dashboard-middle">
        <section className="panel dashboard-chart-panel">
          <header>
            <div>
              <h2>პორტფელის ღირებულება</h2>
              <p>ღირებულების ცვლილება დროში</p>
            </div>
            <div className={pnlClass(s.totalPnl)}>
              <strong>
                {percentage(
                  s.value && s.totalPnl ? percent(s.totalPnl, s.value) : null,
                  true,
                )}
              </strong>
              <span>{money(s.totalPnl)}</span>
            </div>
          </header>
          {history ?? (
        ...
        <section className="panel dashboard-allocation">
          <header>
            <h2>კრიპტო აქტივების განაწილება</h2>
            <Link href={`${base}/allocation`}>ნახვა</Link>
          </header>
          <div className="allocation-body">
            ...
            >
              <div>
                <strong>{money(String(allocationValue), true)}</strong>
                <span>კრიპტო აქტივები</span>
              </div>
            </div>
            <div className="allocation-legend">
              {slices.slice(0, 6).map((slice) => (
                <div key={slice.label}>
                  ...
                  <strong>{percentage(String(slice.value))}</strong>
                </div>
              ))}
            </div>
          </div>
      ...
      <section className="dashboard-lower">
            ...
            <div>
              {action ?? (
                <Link
                  ...
                  href={preview ? "/login" : `${base}/transactions`}
                >
                  <Plus size={15} /> ახალი აქტივი
                </Link>
              )}
            </div>
          </header>
        ...
        <aside className="dashboard-side-stack">
          ...
          <section className="panel dashboard-actions p-4">
            <h2>სწრაფი ქმედებები</h2>
            <div>
              <Link href={`${base}/transactions`}>
                <Plus size={16} /> ახალი აქტივი
              </Link>
              <Link href={`${base}/positions`}>
                <Pencil size={16} /> აქტივის რედაქტირება
              </Link>
              <Link href={`${base}/transactions`}>
                <ArrowLeftRight size={16} /> ტრანზაქცია
              </Link>
              <Link href={`${base}/settings`} className="danger">
                პარამეტრები
              </Link>
            </div>
          </section>
      ...
      <footer className="live-ticker">
        ...
        {positions.slice(0, 5).map((position) => (
          <div key={position.assetId}>
            ...
            <span>{money(position.quote?.price ?? null)}</span>
            <i className={pnlClass(position.quote?.change24h ?? null)}>
        ...
        <small>
          <b />
          Market data: {s.stale ? "Delayed" : "Live"}
        </small>
      </footer>
    </div>
  );
