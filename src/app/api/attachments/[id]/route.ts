...
async function ownedFile(id: string, userId: string) {
  ...
  const [file] = await getDb()
    .select({ file: journalAttachments })
    .from(journalAttachments)
    .innerJoin(portfolios, eq(portfolios.id, journalAttachments.portfolioId))
    .where(and(eq(journalAttachments.id, id), eq(portfolios.userId, userId)));
  return file?.file;
}
export async function GET(
  ...
  if (!user) return new Response("საჭიროა ანგარიშში შესვლა.", { status: 401 });
  try {
    const file = await ownedFile((await params).id, user.id);
    if (!file) return new Response("ფაილი ვერ მოიძებნა.", { status: 404 });
    const blob = await get(file.blobPath, { access: "private" });
    if (!blob || blob.statusCode !== 200)
      return new Response("ფაილი ვერ მოიძებნა.", { status: 404 });
    ...
    return new Response("ფაილის ჩატვირთვა ვერ მოხერხდა.", { status: 500 });
  }
}
export async function DELETE(
  ...
  if (
    ...
    return Response.json({ error: "წვდომა აკრძალულია." }, { status: 403 });
  try {
    const file = await ownedFile((await params).id, user.id);
    if (!file)
