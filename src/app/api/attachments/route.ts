...
export async function POST(request: Request) {
  ...
  if (!process.env.BLOB_READ_WRITE_TOKEN && !process.env.BLOB_STORE_ID)
    return Response.json(
      { error: "დანართების შენახვა დროებით მიუწვდომელია." },
      { status: 503 },
    );
  let uploaded: string | undefined;
  try {
    const reader = request.body?.getReader();
    if (!reader) throw new Error("INVALID_FILE");
    const chunks: Buffer[] = [];
    let size = 0;
    while (true) {
      ...
      if (size > MAX_FILE + 65536) {
        await reader.cancel();
        return Response.json(
          { error: "ფაილის მაქსიმალური ზომაა 2 მბ." },
          { status: 413 },
        );
      }
    ...
    const portfolioId = idSchema.parse(form.get("portfolioId")),
      journalId = idSchema.parse(form.get("journalId"));
    ...
    await db.transaction(async (tx) => {
      ...
      const blob = await put(
        `journals/${portfolioId}/${crypto.randomUUID()}`,
        buffer,
        { access: "private", contentType: file.type, addRandomSuffix: true },
      );
      uploaded = blob.pathname;
      await tx.insert(journalAttachments).values({
        portfolioId,
        journalId,
        name:
          file.name.replace(/[\r\n\x00-\x1f]/g, "").slice(0, 180) || "დანართი",
    ...
    return Response.json(
      {
        error:
          "ფაილი ვერ შეინახა. დასაშვებია PDF, PNG ან JPEG, მაქსიმუმ 2 მბ; თითო ჩანაწერზე — 5 ფაილი.",
      },
      { status: 400 },
    );
  }
