...
export default async function Page({ params }: { params: Promise<{ portfolioId: string }> }) {
  ...
  return <><PageHeading eyebrow={workspace.portfolio.name} title="Airdrops" description="მიღებული აქტივები, საწყისი ღირებულება და ბაზრის ცვლილება." action={<TransactionForm portfolioId={portfolioId} revision={workspace.portfolio.revision} assets={workspace.assets} initialKind="airdrop" triggerLabel="Airdrop-ის დამატება" />} /><AirdropWorkspace entries={workspace.entries} assets={workspace.assets} summary={workspace.summary} portfolioId={portfolioId} revision={workspace.portfolio.revision} /></>;
}
