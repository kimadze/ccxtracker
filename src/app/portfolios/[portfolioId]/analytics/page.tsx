...
export default async function Page({ params }: { params: Promise<{ portfolioId: string }> }) {
  ...
  return <><PageHeading eyebrow={w.portfolio.name} title="ანალიტიკა" description="შედეგები, კონცენტრაცია და პორტფელის მდგომარეობა." /><Analytics summary={w.summary} entries={w.entries} snapshots={history.map(s => ({ ...s, capturedAt: s.capturedAt.toISOString() }))} /></>;
}
