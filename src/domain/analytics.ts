...
export function analyzeHealth(summary: PortfolioSummary) {
  ...
  return {
    ...
    effectivePositions,
    concentration,
    largestSymbol: sorted[0]?.asset.symbol ?? "USD",
  };
}
  ...
  for (let i = 0; i < ordered.length; i++) {
    ...
    if (i) {
      const previous = ordered[i - 1],
        start = Date.parse(previous.capturedAt),
        end = Date.parse(current.capturedAt);
      const interval = entries.filter(
        (e) =>
          Date.parse(e.occurredAt) > start && Date.parse(e.occurredAt) <= end,
      );
    ...
    series.push({
      ...
      value: current.value,
      index: valid ? amount(index) : null,
    });
  }
  return {
    series,
    returnPercent: valid ? amount(index.minus(100)) : null,
    maxDrawdown: valid ? amount(maxDrawdown) : null,
    valid,
  };
}
