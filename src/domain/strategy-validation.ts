  ...
  levels: z
    .array(
      z.object({
        ...
        percentage: positiveAmount.refine(
          (v) => decimal(v).lte(100),
          "წილი არ უნდა აღემატებოდეს 100%-ს.",
        ),
      }),
    )
    .min(1)
...
export const journalSchema = z.object({
  ...
  thesis: z
    .string()
    .trim()
    .max(10000, "ტექსტი არ უნდა აღემატებოდეს 10 000 სიმბოლოს."),
  entryReason: z.string().trim().max(5000),
  catalysts: z.string().trim().max(5000),
  invalidation: z.string().trim().max(5000),
  targets: z.string().trim().max(2000),
  conviction: z.enum(["low", "medium", "high"]),
  horizon: z.string().trim().max(200),
