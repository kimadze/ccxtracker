...
test("statistics preview hydrates without client/server text mismatches", async ({
  ...
  await expect(page.getByRole("heading", { name: "ეკონომიკური მაჩვენებლები" })).toBeVisible();
  await expect(page.getByRole("heading", { name: "ინფლაცია", exact: true })).toBeVisible();
  await expect(page.getByRole("heading", { name: "შრომის ბაზარი", exact: true })).toBeVisible();
  await expect(page.getByRole("heading", { name: "ეკონომიკური კალენდარი" })).toHaveCount(0);
